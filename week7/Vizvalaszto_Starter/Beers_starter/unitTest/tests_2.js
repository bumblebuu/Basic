QUnit.test("Tests for Catalog", function (assert) {
    ...
    sut.setSearch('');
    actual= sut.getItems();
    assert.propEqual(actual, expected, 'Test for setSearch() combined with getItems() and no search text');

    expected = [fakeBeers[0], fakeBeers[2]];
    sut.setSearch('uz');
    actual= sut.getItems();
    assert.propEqual(actual, expected, 'Test for setSearch() combined with getItems()');
});
